    ...
    public class Act {
        ...
        public static void Input_Switch_Behaviour(InputAction.CallbackContext callbackContext) {
            if (callbackContext.performed) {
                SwitchBehaviour(callbackContext.ReadValue<Vector2>());
            }
        }

        private static void SwitchBehaviour(Vector2 scrollVec2) {
            ...
            Transform grid = UI_Main.Instance.UIMainComp.Get<Transform>(string.Concat("UI_Main_Behaviour_Icon_", index.ToString()));
            select.parent = grid;
            select.localPosition = Vector3.zero;
            UI_Main.Instance.DisplaySelectBehaviour();
            Sound.Instance.PlaySound("Switch", Vector3.zero, false);
        }
